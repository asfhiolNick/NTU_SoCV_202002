...
void V3Ntk::buildBdd(const V3NetId& netId) {
  ...
  //cout << "orderedNets.size() = " << orderedNets.size() << endl;
  for(int i=0, n=orderedNets.size(); i<n; ++i){
    V3NetId& gateid = orderedNets[i];
    if(getGateType(gateid) == AIG_NODE){
      BddNodeV a = bddMgrV->getBddNodeV(gateid.id);
      BddNodeV l = bddMgrV->getBddNodeV(getInputNetId(gateid, 0).id);
      if(getInputNetId(gateid, 0).cp) l = ~l;
      BddNodeV r = bddMgrV->getBddNodeV(getInputNetId(gateid, 1).id);
      if(getInputNetId(gateid, 1).cp) r = ~r;
      a = l & r;
      bddMgrV->addBddNodeV(gateid.id, a());   
    }
  }
}
